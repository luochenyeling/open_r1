"""
Piston客户端模块，用于与Piston服务器进行交互，实现代码执行和评分功能。

主要功能：
1. 自动负载均衡多个Piston工作节点
2. 执行代码并获取评分结果
3. 管理Piston包的安装和卸载
4. 处理执行错误和重试机制

主要组件：
1. PistonError: 自定义异常类
2. get_piston_client_from_env: 从环境变量获取Piston客户端
3. PistonClient: Piston客户端主类
4. get_slurm_piston_endpoints: 从SLURM获取Piston工作节点列表
"""
...
def get_piston_client_from_env():
    ...
    从PISTON_ENDPOINTS环境变量获取Piston服务器端点列表，
    支持从SLURM队列系统获取端点或直接指定端点列表
    
    返回:
        PistonClient: 配置好的Piston客户端实例
        
    异常:
        ValueError: 当PISTON_ENDPOINTS环境变量未设置时抛出
    """
...
class PistonClient:
    """
    Piston客户端类，实现自动负载均衡和代码执行功能
    
    该类管理多个Piston工作节点，自动处理负载均衡和错误重试。
    使用自定义的cms_ioi包进行代码执行和评分。
    
    建议使用以下脚本启动Piston实例以避免IOI问题的默认限制：
    ```
    ...
    def __init__(
        ...
        参数:
            base_endpoint: Piston服务器端点或端点列表
            session: aiohttp会话对象
            max_requests_per_endpoint: 每个端点的最大并发请求数
        """
    ...
    async def execute(self, data) -> tuple[str, str]:
        ...
        IOI包在stdout中返回浮点数形式的分数，在stderr中返回可选的反馈/错误信息
...
def get_slurm_piston_endpoints():
    """
    从SLURM队列系统获取活动的Piston工作节点端点列表
    
    通过squeue命令获取作业名称、主机名和状态，过滤出RUNNING状态的作业
    
    返回:
        list: Piston工作节点端点URL列表
    """
